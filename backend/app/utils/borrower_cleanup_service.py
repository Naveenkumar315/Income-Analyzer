...
def similarity_score(name1: str, name2: str) -> float:
    ...
    p1, p2 = extract_name_components(name1)['all_parts'], extract_name_components(name2)['all_parts']
    if not p1 or not p2:
        return direct
...
def extract_borrower_name_from_document(doc: Dict[str, Any]) -> Optional[str]:
    ...
    borrower_name = None
    if 'Summary' in doc:
        summaries = doc['Summary'] if isinstance(doc['Summary'], list) else [doc['Summary']]
    ...
    if 'Summary' in doc:
        ...
        for s in summaries:
            for label in s.get('Labels', []):
                ...
                if not name:
                    continue
                vals = [v.get('Value') for v in label.get('Values', []) if 'Value' in v]
                if vals:
                    ...
                    for g in label['Groups']:
                        ...
                        for rl in g.get('RecordLabels', []):
                            ...
                            rvals = [v.get('Value') for v in rl.get('Values', []) if 'Value' in v]
                            if rname and rvals:
                                record[rname] = rvals[0] if len(rvals) == 1 else rvals
                        if len(record) > 1:
    ...
    for item in items:
        ...
        for dtype, value in item.items():
            ...
            for doc in docs:
                ...
                if cdoc:
                    cleaned.setdefault(match, {}).setdefault(dtype, []).append(cdoc)
