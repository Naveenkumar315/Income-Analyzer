...
const LoanExatraction = () => {
    ...
    return (<>
        <div className="flex justify-between items-center p-2 rounded-lg">
            {/* Left side: filename */}
            <span className="font-medium">Upload & Extract Files (LN-20250915-001)</span>

            {/* Right side: buttons */}
            <div className="flex gap-2">
                <Button variant="upload-doc" width={200} label={"Upload Documents"} />
                <Button variant="start-analyze" width={200} label={"Start Analyzing"} />
            </div>
        </div>
        ...
        ${item === selected ? "border-l-4 border-[#26a3dd] font-medium bg-gray-100 rounded-r-md" : "border-gray-200"}
      `}
                        >
                            {item}
                        </li>
                    ...
                    {documents.map((doc, idx) => (
                        <div key={idx} className="border border-gray-200 rounded-lg shadow-sm bg-white">
                            <div className="flex  items-center gap-[30px] font-medium p-2 bg-gray-100 rounded-t-lg cursor-pointer">
                                ...
                                <span className="text-sm text-gray-500">{doc.fieldCount} Fields Extracted</span>
                            </div>
